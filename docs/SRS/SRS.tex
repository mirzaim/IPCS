...
% #8 Referenced by
{
  \iref{I_cart_motion}, \iref{I_pendulum_motion}
}
% #9 Preconditions
{
...
\hline
Equation&$\vec{r}_P = (x-l\sin\theta)\hat{x}_I + (l\cos\theta) \hat{y}_I$  \\
\hline
Description &
The position of the pendulum in 2D space is given by the above equation, where
$\vec{r}_P$ is the position vector of the pendulum, $x$ is the
position of the cart (\si{\metre}), $l$ is the length of the rod (\si{\metre}),
and $\theta$ is the angle of the pendulum arm with respect to the vertical axis
(\si{\radian}).
\\
\hline
  Source & N/A \\
...
\aref{A_masslessRod}              & & &X& &X&X\\ \hline
\aref{A_classicalMechanics}       &X& & & &X&X\\ \hline
\aref{A_noFriction}               & &X&X& &X&X\\ \hline
\aref{A_rigidBody}                & &X&X& &X&X\\
\hline
\end{tabular}
