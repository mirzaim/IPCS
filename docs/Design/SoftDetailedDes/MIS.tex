...
\section{MIS of Config Reader Module} \label{mConfigReader} \wss{Use labels for
  cross-referencing}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
load\_config & file\_path: $\text{char}^*$ & World & FileNotFoundException \\
% load\_Fuzzy\_logic & file\_path: $\text{char}^*$ & 
...
% \begin{itemize}
% \item transition: -
% \item output: It acts like a compiler and reads the file and returns the 
...
\section{MIS of World Checker Module} \label{mOutputVerification} \wss{Use labels for
  cross-referencing}
...
\item exception: 
      $exc := ( \\
      \text{if not } (\text{all parameters in world is float}) \text{ then }
      \text{ValueError} \\
      | \\
      \text{if not } (0 \leq m \leq 80) \text{ then }
      \text{ValueError} \\
      | \\
      \text{if not } (0 \leq M \leq 80) \text{ then }
      \text{ValueError} \\
      | \\
      \text{if not } (0 \leq l \leq 20) \text{ then }
      \text{ValueError} \\
      | \\
      \text{if } (\text{min\_x} \leq \text{x} \leq \text{max\_x}) 
      \text{ then } \text{ValueError}\\
      )$
\end{itemize}
...
\section{MIS of Manager Module} \label{mManager} \wss{Use labels for
  cross-referencing}
...
Simulator Module (Section \ref{mSimulator}) \\
Controller Module (Section \ref{mController})
...
\section{MIS of Main Module} \label{mMain} \wss{Use labels for
  cross-referencing}
...
Config Reader Module (Section \ref{mConfigReader}) \\
World Module (Section \ref{mWorld}) \\
...
\begin{itemize}
\item transition:
      This is the main function of the program. It follows the following steps:
      \begin{itemize}
        \item Read the configuration file using Config Reader.
        \item Create the world using the configuration.
        \item Create the Manager, Simulator, and Controller.
        \item Call the run() method of the Manager.
      \end{itemize}
\item output: \wss{if appropriate} 
\item exception: \wss{if appropriate} 
\end{itemize}
...
\section{MIS of File Output Module} \label{mFileOutput} \wss{Use labels for
  cross-referencing}
...
\begin{itemize}
\item transition: -
\item output: It returns true for n steps of simulation time.
\item exception: -
\end{itemize}

\noindent draw(world):
\begin{itemize}
\item transition: Write the current world state to a file.
\item output: -
\item exception: -
\end{itemize}
...
\noindent quit():
\begin{itemize}
\item transition: Closes the file.
\item output: -
\item exception: -
\end{itemize}
